...
interface RenderFilterProps {
    ...
    onChangeFilters: (filters: string[] | undefined) => void;
}
...
export function RenderFilter({ activeFilters = [], onChangeFilters }: RenderFilterProps) {
    ...
    const handleCheckboxChange = (sc: string, isChecked: boolean) => {
        ...
        if (isChecked) {
            updatedFilters = [...currentFilters, sc];
        } else {
            updatedFilters = currentFilters.filter(item => item !== sc);
        }

        onChangeFilters(updatedFilters.length > 0 ? updatedFilters : undefined);
    };
                        ...
                        {category.map(c =>
                            c.subcategories.map(sc => (
                                ...
                                transition-all duration-300 ease-in-out rounded px-2">
                                    ...
                                    />
                                    <label htmlFor={sc} className="cursor-pointer text-lg sm:text-xl select-none w-full"
                                    >{sc}</label>
                                </div>
                            ))
                        )}
